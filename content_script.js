...
// --- Summary diff view (all files listed) ---
...
// --- Single-file view (Monaco editor — diff and non-diff) ---

/**
 * Extract the filename for the single-file view from the toolbar header.
 * The filename lives in: .repos-compare-toolbar span.text-ellipsis[role="heading"]
 * This is used for both diff views and non-diff views (e.g. new files).
 */
function getMonacoDiffFileName() {
...
function getThemeProbe(themeClass) {
  if (themeProbes.has(themeClass)) return themeProbes.get(themeClass);
...
function processMonacoViewLines(viewLinesElement, language, themeClass) {
  ...
  viewLines.forEach(viewLine => {
    ...
    if (monacoSpans.length === 0) return;
    ...
    let tokenIdx = 0;
    for (const ms of monacoSpans) {
      if (ms.isWhitespace) continue; // Don't color whitespace markers
      if (ms.node.nodeType !== Node.ELEMENT_NODE) continue; // Skip bare text nodes
...
function processMonacoEditors() {
  const fileName = getMonacoDiffFileName();
  const language = getLanguageFromFileName(fileName);
  if (!language) {
...
function applySyntaxHighlighting() {
  ...
  // Summary diff view (all changed files listed with inline diffs)
  ...
  // Single-file view (Monaco editor — both diff and non-diff)
...
// added/removed/recycled as the user scrolls. We use requestAnimationFrame
...
function scheduleMonacoHighlighting() {
  if (monacoRafPending) return;
  monacoRafPending = true;
  requestAnimationFrame(() => {
    monacoRafPending = false;
    if (!window.location.href.includes('/_git/')) return;
    processMonacoEditors();
  });
}
